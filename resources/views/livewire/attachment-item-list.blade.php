    ...
    <template x-if="typeof attachments !== undefined">
        <template x-for="(attachment, index) in attachments">
            <div @click="$store.attachmentBrowser?.handleItemClick(attachment, statePath)"
                 ...
                 :class="{'bg-black dark:bg-white dark:text-black text-white': (attachment?.type === 'attachment' && $store.attachmentBrowser?.isSelected(attachment.id, statePath))}"
                ...
                <template x-if="attachment?.type === 'attachment' && attachment.isImage">
                ...
                <div class="absolute p-6">
                    ...
                    <template x-if="attachment?.type === 'attachment' && attachment.isImage"><x-filament::icon icon="heroicon-o-photo" class="w-8 h-8 m-0" /></template>
                    <template x-if="attachment?.type === 'attachment' && attachment.isVideo"><x-filament::icon icon="heroicon-o-film" class="w-8 h-8 m-0" /></template>
                    <template x-if="attachment?.type === 'attachment' && (!attachment.isImage && !attachment.isVideo)"><x-filament::icon icon="heroicon-o-document" class="w-8 h-8 m-0" /></template>
                ...
                <div class="absolute top-4 right-4" x-on:click.stop="">
                    <x-filament::dropdown placement="bottom-end" x-show="$store.attachmentBrowser?.showActions(statePath)">
                        <x-slot name="trigger">
                            <x-filament::icon
                                x-show="
                                (attachment?.type === 'attachment' && !$store.attachmentBrowser?.isSelected(attachment.id, statePath))
                        ...
                        <x-filament::dropdown.list x-show="attachment?.type === 'attachment'">
                        ...
                        <x-filament::dropdown.list x-show="attachment?.type === 'directory'">
