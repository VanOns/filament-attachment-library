        ...
        <template x-for="(attachment, index) in attachments">
            <div @click="$store.attachmentBrowser?.handleItemClick(attachment, statePath)"
                ...
                <div class="absolute p-6">
                    ...
                    <template x-if="attachment?.class === 'attachment' && attachment.isImage"><x-filament::icon icon="heroicon-o-photo" class="w-8 h-8 m-0" /></template>
                    <template x-if="attachment?.class === 'attachment' && attachment.isVideo"><x-filament::icon icon="heroicon-o-film" class="w-8 h-8 m-0" /></template>
                    <template x-if="attachment?.class === 'attachment' && (!attachment.isImage && !attachment.isVideo)"><x-filament::icon icon="heroicon-o-document" class="w-8 h-8 m-0" /></template>
